...
public class ChessGame {
	...
	public ChessGame(@NonNull Reader input) throws IOException {
		...
		while((line = reader.readLine()) != null) {
			...
			String whiteMoveStr = line.substring(0,pos);
			assert(whiteMoveStr != null);		//checks for not equal null
			Move white = moveFromString(whiteMoveStr,true);

			Move black = null;
			if(pos != line.length()) {
				String blackMoveStr = line.substring(pos+1);
				assert(blackMoveStr != null);	//checks for not equal null
				black = moveFromString(blackMoveStr,false);
			}
	...
	public @NonNull List<Board> boards() {
		...
		for(Round r : rounds) {		
			...
			//checks if white moves != null then apply the moves
			Move whiteMove = r.white();
			assert(whiteMove != null);
			if(!b.apply(whiteMove)) { return boards; }
			boards.add(b);	

			//checks if black moves != null then apply the moves
	...
	private static @NonNull Move moveFromString(@NonNull String str, boolean isWhite) {
		...
		//checks if the start of the string does not equal null
		String startString = str.substring(index,index+2);
		assert(startString != null);
		Position start = positionFromString(startString);
		...
		//checks if the end of the string does not equal null
		String endString = str.substring(index,index+2);
		assert(endString != null);
		Position end = positionFromString(endString);
